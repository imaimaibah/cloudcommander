...
sub main($$){
	...
	my $num = activeCheck();
	system("expect -f $base_dir/exp/ssl_vpn.exp $ipcom[$num] $region_num > $base_dir/log/exp/ssl_vpn-$ipcom[$num].log");
	$ret = $?;
	$ret>>8;
...
EOF
	...
	foreach my $user(@userID){
		system("$base_dir/pl/smtp.pl -f 'GCP_Administrator\@jp.fujitsu.com' -t '$user' -c 'c3s-global\@ml.css.fujitsu.com' -s 'Your SSL-VPN password has been initialized.' -d '$base_dir/tmp/initial_mail.txt'");
	}
