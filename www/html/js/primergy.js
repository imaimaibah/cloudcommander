...
//	$("#events").html("");
	...
	sessionStorage.setItem("selected_host_status",st);
	sessionStorage.setItem("selected_host_ip",ip);
...
function enclosureHTML_SVRM(list,id){
	...
	var dom0_status = sessionStorage.getItem("selected_host_status");
	if(vmflag != 0 && island != "region" && dom0_status == "PrimNG.jpg"){
...
function migration(){
	...
	var dom0ip = sessionStorage.getItem("selected_host_ip");
	var _div = document.getElementById("islandList");
	var island = _div.options[_div.selectedIndex].value+"-cbrm";
	var url = "/cgi-bin/resource/migration_cmd.cgi?dom0="+dom0+"&island="+island+"&dom0ip="+dom0ip;
