...
open(IN,"/data/cgi-bin/network/pickup_vnet_id") or die "FAILED";
while(<IN>){
	chomp;
	s/\r//g;
...
open(IN,"/data/cgi-bin/network/pickup_vlan_id") or die "FAILED";
$vlanid = substr(<IN>,28);
close(IN);
...
open(IN,"/data/cgi-bin/network/vrf_serach_1") or die "FAILED";
while(<IN>){
	chomp;
	s/\r//g;
  my @tmp = split(/\s+/);
...
open(IN,"/data/cgi-bin/network/vrf_serach_2") or die "FAILED";
...
#    open(IN2,"$base_dir/exp/network/vrf_serach_3.exp 10.3.1.3 $int|") or die "FAILED";
    open(IN2,"/data/cgi-bin/network/vrf_serach_3_1") or die "FAILED";
    while(<IN2>){
      chomp;
      s/\r//g;
      if (/description/){
